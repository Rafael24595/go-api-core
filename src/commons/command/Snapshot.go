...
func runSnapshotDetails(flag string, cmd *collection.Vector[string]) (string, error) {
	...
	exists := collection.VectorFromList(listeners).
		Contains(func(t system.TopicSnapshot) bool {
			return t == topic
		})
	...
	if path == "" {
		return "", fmt.Errorf("undefined path for type %q", typ)
	}

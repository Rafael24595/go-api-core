...
func NewResponseBody(contentType domain.ContentType, payload string) *BodyResponse {
	return &BodyResponse{
...
func EmptyResponseBody(contentType domain.ContentType) *BodyResponse {
	return NewResponseBody(contentType, "")
}

func EmptyBody(status bool, contentType domain.ContentType) *BodyRequest {
	return NewBody(status, contentType, make(map[string]map[string][]BodyParameter))
}

func NewBody(status bool, contentType domain.ContentType, parameters map[string]map[string][]BodyParameter) *BodyRequest {
	return &BodyRequest{
		Status:      status,
		ContentType: contentType,
...
func (b BodyRequest) Empty() bool {
	return b.ContentType == domain.None
}

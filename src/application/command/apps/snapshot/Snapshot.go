...
var refSave = apps.CommandReference{
	...
	Example:     fmt.Sprintf(`%s %s ${topic}=${type}`, Command, FLAG_SAVE),
}
...
var refApply = apps.CommandReference{
	...
	Example:     fmt.Sprintf(`%s %s ${topic}=${name}.${extension}`, Command, FLAG_APPLY),
}
...
var refRemove = apps.CommandReference{
	...
	Example:     fmt.Sprintf(`%s %s ${topic}=${name}.${extension}`, Command, FLAG_REMOVE),
}
